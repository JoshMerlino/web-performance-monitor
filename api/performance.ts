...
declare type ReqStats = { req_counter: number, req_per_second: number };
...
export async function profile(): Promise<void> {
	...
	try {
		...
		const requests = JSONStore.from<ReqStats>("~/.proxy/stats.json").value;
...
export default function api(_req: Request, res: Response): void {
	(function runtime(): void | NodeJS.Timeout {
		if (response.success !== true) return setTimeout(runtime, 200);
		res.json(response);
	}());
}
